...
def register():
    ...
    try:
        cur.execute(
            sql.SQL("INSERT INTO users (email, username, password, balance, telegram_id, name, age, identity_card) VALUES (%s, %s, %s, %s, %s, %s, %s, %s) RETURNING id"),
            (email, username, hashed, 0.0, data.get('telegram_id'), data.get('name'), data.get('age'), data.get('identity_card'))
        )
...
def login():
    ...
    try:
        ...
        return jsonify({'error': 'Error al iniciar sesión', 'details': str(e)}), 400
    finally:
        cur.close()
        conn.close()
...
def buy_balance():
    ...
    try:
        cur.execute(
            sql.SQL("INSERT INTO transactions (sender_id, receiver_wallet, amount, status) VALUES (%s, %s, %s, %s) RETURNING id"),
            (user_id, Config.ADMIN_WALLET, amount, 'pending')
        )
        tx_id = cur.fetchone()[0]
        conn.commit()
        return jsonify({
            'message': 'Solicitud de compra de saldo creada',
            'transaction': {
                'id': tx_id,
                'sender_id': user_id,
...
def get_transactions(user_id):
    ...
    try:
        cur.execute(
            sql.SQL("SELECT id, sender_id, receiver_wallet, amount, status, created_at, tx_hash FROM transactions WHERE sender_id = %s"),
            (user_id,)
        )
        transactions = cur.fetchall()
        result = [
            {'id': t[0], 'sender_id': t[1], 'receiver_wallet': t[2], 'amount': t[3], 'status': t[4], 'created_at': t[5].isoformat(), 'tx_hash': t[6]}
...
def get_offers():
    ...
    try:
        cur.execute(
            sql.SQL("SELECT id, user_id, type, amount, min_amount, max_amount, comment, status, created_at FROM offers WHERE status = %s"),
            ('active',)
        )
        offers = cur.fetchall()
        result = [
            {'id': o[0], 'user_id': o[1], 'type': o[2], 'amount': o[3], 'min_amount': o[4], 'max_amount': o[5], 'comment': o[6], 'status': o[7], 'created_at': o[8].isoformat()}
            for o in offers
        ]
        return jsonify(result)
    except Exception as e:
...
def create_offer():
    ...
    if not all([offer_type, amount, min_amount, max_amount, comment]) or amount <= 0 or min_amount <= 0 or max_amount <= min_amount:
        return jsonify({'error': 'Datos inválidos'}), 400
    conn = get_db_connection()
    cur = conn.cursor()
    try:
        cur.execute(
            sql.SQL("INSERT INTO offers (user_id, type, amount, min_amount, max_amount, comment, status) VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING id"),
            (user_id, offer_type, amount, min_amount, max_amount, comment, 'active')
        )
